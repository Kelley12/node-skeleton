...
import { isComponent } from "./type-guards";

/**
 * String, Element or and array of Elements
 * to put inside of an html tag
 */
export type HTMLContent = string | HTMLElement | Component |
...
function fillElement(element: HTMLElement, content?: HTMLContent): void {
    if (!content) return;
...
function setElementAttributes(element: HTMLElement, attributes?: Attributes): void {
    if (attributes) {
 ...
 */
export function mount(el: HTMLElement, selector: string): boolean {
...
/** Remove everything from the inside of an HTMLElement */
export function removeChildrenFromHTMLElement(el: string | HTMLElement): void {
    let elActual: HTMLElement | null;
